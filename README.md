...
- Provenance documentation using PROV-O (`provenance/provenance.ttl`).
...
7. Acknowledgements extraction from the PMC OAI API
   ...
   - The PMC OAI API applies rate limiting. If the script returns 0 results,
     wait 30-60 minutes before retrying. The pre-generated output is available
     at `data/processed/acknowledgements.csv`.
...
The full provenance of the research pipeline is documented using the W3C PROV Ontology (PROV-O):

```text
provenance/provenance.ttl
```

This file records all pipeline activities (A01–A10), the data entities consumed and produced at each step, the software agents used, and the delegation relationships between agents and the student group.
...
conda activate sports-injuries-kg
```
...
git clone https://github.com/estebanmoreno04/open-science-sports-injuries-kg.git
cd open-science-sports-injuries-kg
docker build -t sports-injuries-kg .
...
Or run the full pipeline and explorer together:
...
The repository includes a `.env.example` file as a template — this file is safe
to commit because it contains no real credentials. The actual `.env` file where
you put your credentials is listed in `.gitignore` and is **never committed**.
...
> **Why two files?** `.env.example` documents what credentials are needed so
> anyone cloning the repo knows exactly what to set up. `.env` contains your
> actual secrets and is never shared. This is standard practice for reproducible
> research software.
...
The justification for all model choices (embedding model, topic modeling methods, NER model) is documented in:
...
- The PMC OAI API applies rate limiting. The pre-generated `acknowledgements.csv`
  is included in the repository to ensure reproducibility without depending on
  API availability.
...
- Adding an RO-Crate package for full FAIR compliance.
- Integrating a persistent SPARQL endpoint for public access to the Knowledge Graph.
